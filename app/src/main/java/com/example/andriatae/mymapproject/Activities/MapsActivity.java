...
import com.example.andriatae.mymapproject.Network.MyFirebaseMessagingService;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
  ...
  application=myApplication.get();
    ...
    //carPark=Realm.getInstance();
        ...
        isinitialcomplete=presenter.initialCarparkFindings();
    ...
    public void createMarkers(){
      ...
      //  System.out.println("the size of the array of coords "+ myLatLng.size());
        ...
        for(CarPark l:myResults){
           ...
           if (l.getIsReserved().equals(false))
            {
               ...
               Marker marker = mMap.addMarker(new MarkerOptions().position(lit).title(l.getName()).snippet("This Car Park is notavailable \n<*> Reservation information:<*> \nMinimum Time  Maximum Time  Cost\n      "+l.getMinReserveTimeMins()+" Minutes       " +l.getMaxReserveTimeMins()+ "Minutes      "+"$"+l.getCostPerMinute()+""+"\n Booked Until:"+l.getReservedUntil().toString()).zIndex(i));
                                                ...
                                                public void onTabSelected(TabLayout.Tab tab) {
                                                    switch (tab.getPosition()) {
                                                        case 0:
                                                           ...
                                                           // Intent myIntentgoback = new Intent(ListReservations.class,Maps)
...
if(isinitialcomplete) {
    ...
    System.out.println("Havent had time to create a callback for the initial setup stage " +
            "if this bool on create markers doesnt work then a workaround to make it work would be" +
            "to comment out create markers when you first downlaod the app then uncomment");
}
